...
class M_penjualan_master extends CI_Model
{
	...
	function fetch_data_penjualan($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			SELECT 
				...
				CONCAT('Rp. ', REPLACE(FORMAT(a.`grand_total`, 0),',','.') ) AS total_bayar,
	...
	function get_detail($id_penjualan){
		...
		$this->db->select('md.*, mn.*')->from('pj_penjualan_detail md');
		$this->db->join('pj_barang mn', 'md.id_menu=mn.id_barang', 'left');
		$this->db->where('md.id_penjualan_m', $id);
	...
	function laporan_pembelian($from, $to){
		...
		// $this->db->sselect('DISTINCT(tanggal) as tanggal,  ')
		$sql = "
			SELECT 
				DISTINCT(SUBSTR(a.`tanggal`, 1, 10)) AS tanggal,
				(
	...
	function penjualan($tgl){
		$this->db->where('date(tanggal)', $tgl);

		$res = $this->db->get('pj_penjualan_master')->result_array();
	...
	function detail($id){
		...
		$this->db->select('d.*, m.nama_barang as menu')->from('pj_penjualan_detail d');
		$this->db->join('pj_barang m', 'd.id_menu=m.id_barang');
		$this->db->where('d.id_penjualan_m', $id);
		$res = $this->db->get()->result_array();
