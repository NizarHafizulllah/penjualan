...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5><i class='fa fa-cube fa-fw'></i> Pengeluaran <i class='fa fa-angle-right fa-fw'></i> Data</h5>
			<hr />
...
{
	$tambahan .= nbs(2)."<a href='".site_url('pengeluaran/tambah')."' class='btn btn-default' id='TambahPengeluaran'><i class='fa fa-plus fa-fw'></i> Tambah Pengluaran</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('pengeluaran/pengeluaran_json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#TambahPengeluaran, #EditPengeluaran', function(e){
		e.preventDefault();
		if($(this).attr('id') == 'TambahPengeluaran')
		{
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').addClass('modal-lg');
			$('#ModalHeader').html('Tambah Pengeluaran');
		}
		if($(this).attr('id') == 'EditPengeluaran')
		{
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').removeClass('modal-lg');
			$('#ModalHeader').html('Edit Pengeluaran');
		}
	...
	$(document).on('keyup', '.kode_barang', function(){
		...
		if(Pass > 0)
		{
			...
			$('#SimpanTambahPengeluaran').addClass('disabled');
		}
		else
		{
			$(this).parent().find('span').html('');
			...
			$.ajax({
				...
				success: function(json){
					if(json.status == 0){ 
						$('#TabelTambahPengeluaran tbody tr:eq('+Indexnya+') td:nth-child(2)').find('span').html(json.pesan);
						$('#SimpanTambahPengeluaran').addClass('disabled');
					}
					if(json.status == 1){ 
						$('#SimpanTambahPengeluaran').removeClass('disabled');
