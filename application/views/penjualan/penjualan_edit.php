		...
		<div class="panel-body">
			...
			<div class='row'>
						...
						<div class="panel-body">
							...
							<div class="form-horizontal">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input type='text' name='nomor_nota' class='form-control input-sm' id='nomor_nota' value="<?php echo $m_transaksi['nomor_nota']; ?>" <?php echo $readonly; ?>><input type="hidden" name="id_penjualan_m" id="id_penjualan_m" value="<?php echo $m_transaksi['id_penjualan_m']; ?>">
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4 control-label">Tanggal</label>
									<div class="col-sm-8">
										<input type='text' name='tanggal' class='form-control input-sm' id='tanggal' value="<?php echo $m_transaksi['tanggal']; ?>" <?php echo $disabled; ?>>
									</div>
								</div>
								<div class="form-group">
				...
				<div class='col-sm-9'>
					...
					<div class='row'>
						<div class='col-sm-7'>
							<textarea name='catatan' id='catatan' class='form-control' rows='2' placeholder="Catatan Transaksi (Jika Ada)" style='resize: vertical; width:83%;'><?php echo $m_transaksi['catatan']; ?></textarea>
						...
						<div class='col-sm-5'>
							<div class="form-horizontal">
								...
								<div class='row'>
									...
									<div class='col-sm-6'>
										<button type='button' class='btn btn-primary btn-block' onclick="SimpanTransaksi()">
...
$(document).ready(function(){
	...
	<?php if(!empty($detail_transaksi)){ 
		...
		foreach ($detail_transaksi as $k) { 
			$DiskonVal = ($k['diskon']/100)*$k['harga_satuan'];
			$sub_total = ($k['harga_satuan']*$k['jumlah_beli'])-$DiskonVal;
			?>
		
		
		BarisBaru();

			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(1) input').val(<?php echo $k['id_penjualan_d'] ?>);
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(2) input').val('<?php echo $k['kode_barang'] ?>');
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(3)').html('<?php echo $k['nama_barang']; ?>');
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(4) input').val('<?php echo $k['diskon'] ?>');
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(4) span').html(<?php echo $k['diskon'] ?>);
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(5) input').val(<?php echo $k['harga_satuan'] ?>);
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(5) span').html(to_rupiah(<?php echo $k['harga_satuan'] ?>));
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(6) input').removeAttr('disabled').val(<?php echo $k['jumlah_beli'] ?>);
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(7) input').val(<?php echo $sub_total; ?>);
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(7) span').html(to_rupiah(<?php echo $sub_total ?>));
...
function BarisBaru()
{
	...
	var Baris = "<tr>";
		Baris += "<td>"+Nomor+"<input type='hidden' name='id_penjualan_d[]' ></td>";
		Baris += "<td>";
			Baris += "<input type='text' class='form-control kode_barang' name='kode_barang[]' id='pencarian_kode' placeholder='Ketik Kode / Nama Barang'>";
			Baris += "<div id='hasil_pencarian'></div>";
		Baris += "</td>";
		Baris += "<td><span class='nama_barang'></span></td>";
		Baris += "<td>";
			Baris += "<input type='hidden' name='diskon[]' class='diskon'>";
			Baris += "<span></span>";
		Baris += "</td>";
		Baris += "<td>";
			Baris += "<input type='hidden' name='harga_satuan[]' class='harga_satuan'>";
			Baris += "<span></span>";
		Baris += "</td>";
		Baris += "<td><input type='text' class='form-control jumlah_beli' id='jumlah_beli' name='jumlah_beli[]' onkeypress='return check_int(event)' disabled></td>";
		Baris += "<td>";
...
$(document).on('keyup', '#pencarian_kode', function(e){
	if($(this).val() !== '')
	{
		...
		else if(charCode == 13)
		{
			...
			var Diskonnya = Field.find('div#hasil_pencarian li.autocomplete_active span#diskonnya').html();
			...
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(4) input').val(Diskonnya);
			$('#TabelTransaksi tbody tr:eq('+$(this).parent().parent().index()+') td:nth-child(4) span').html(Diskonnya);
...
$(document).on('click', '#daftar-autocomplete li', function(){
	...
	var Diskon = $(this).find('span#diskonnya').html();
	var Harganya = $(this).find('span#harganya').html();
	var DiskonVal = (Diskon/100)*Harganya;
	var SubTotal = Harganya-DiskonVal;
	...
	$('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(4) input').val(Diskon);
	$('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(4) span').html(Diskon);
	...
	$('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(7) input').val(SubTotal);
	$('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(7) span').html(to_rupiah(SubTotal));
...
$(document).on('keyup', '#jumlah_beli', function(){
	// console.log($('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(4) input').val());
	var Indexnya = $(this).parent().parent().index();
	var Harga = $('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(5) input').val();
	var Diskon = $('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(4) input').val();
		...
		success: function(data){
			...
			{	
				if (parseInt(Diskon == 0)) {
					...
					var totalDis = (parseInt(Diskon)/100) * (parseInt(Harga) * parseInt(JumlahBeli));
					var SubTotal = (parseInt(Harga) * parseInt(JumlahBeli)) - parseInt(totalDis);
...
function SimpanTransaksi()
{
	...
	FormData += "&id_penjualan_m="+encodeURI($('#id_penjualan_m').val());
	...
	$.ajax({
		url: "<?php echo site_url('penjualan/update'); ?>",
		type: "POST",
		cache: false,
		data: FormData,
		dataType:'json',
