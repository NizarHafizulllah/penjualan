				...
				<table id="my-grid" class="table table-striped table-bordered">
					<thead>
						<tr>
							...
							<th class='no-sort'>Tambah Barang</th>
							<?php } ?>
						</tr>
					</thead>
				</table>
...
{
	$tambahan .= nbs(2)."<a href='".site_url('stok/tambah')."' class='btn btn-default' id='TambahStok'><i class='fa fa-plus fa-fw'></i> Tambah Bahan</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('stok/stok-json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#TambahStok, #EditBarang', function(e){
		e.preventDefault();
		if($(this).attr('id') == 'TambahStok')
		{
