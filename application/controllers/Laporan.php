...
class Laporan extends MY_Controller 
{
	...
	public function penjualan($from, $to)
	{
		...
		$dt['pengeluaran']	= $this->m_penjualan_master->laporan_pembelian($from, $to);
		$dt['from']			= date('d F Y', strtotime($from));
		$dt['to']			= date('d F Y', strtotime($to));

		// echo '<pre>';
		// print_r($dt['pengeluaran']);
		// echo '<pre>';
		// exit();
		// print_r($dt['penjualan']->result());
	...
	function detail(){
		...
		$dt['tgl'] = $get['tgl'];
		$this->load->model('m_penjualan_master');
		$data = $this->m_penjualan_master->penjualan($get['tgl']);

		foreach ($data as $key => $value) {
			
			$data[$key]['detail'] = $this->m_penjualan_master->detail($value['id_penjualan_m']);
		}
		$dt['data'] = $data;
		// echo '<pre>';
		// print_r($data);
		// echo '</pre>';
		// exit();
		$this->load->view('laporan/detail_laporan_penjualan', $dt);
	...
	function detail_pengeluaran(){
		...
		$this->db->where('tanggal', $dt['tgl']);

		$dt['data'] = $this->db->get('barang_masuk')->result_array();


		$this->load->view('laporan/detail_laporan_pengeluaran', $dt);
	}
...
public function excel_pengeluaran($from, $to)
	{
		$this->load->model('m_penjualan_master');
		$penjualan 	= $this->m_penjualan_master->laporan_pembelian($from, $to);
		if($penjualan->num_rows() > 0)
		{
			$filename = 'Laporan_Pengeluaran_'.$from.'_'.$to;
			header("Content-type: application/x-msdownload");
			header("Content-Disposition: attachment; filename=".$filename.".xls");

			echo "
				<h4>Laporan Pengeluaran Tanggal ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to))."</h4>
				<table border='1' width='100%'>
					<thead>
						<tr>
							<th>No</th>
	...
	public function pdf_pengeluaran($from, $to)
	{
		...
		$pdf->Cell(0, 8, "Laporan Pengeluaran Tanggal ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to)), 0, 1, 'L'); 

		$pdf->Cell(15, 7, 'No', 1, 0, 'L'); 
		$pdf->Cell(85, 7, 'Tanggal', 1, 0, 'L');
		$pdf->Cell(85, 7, 'Total Pengeluaran', 1, 0, 'L'); 
		$pdf->Ln();
		...
		foreach($penjualan->result() as $p)
		{
			...
			$pdf->Cell(85, 7, "Rp. ".str_replace(",", ".", number_format($p->total_pembelian)), 1, 0, 'L');
			$pdf->Ln();
		...
		$pdf->Cell(100, 7, 'Total Seluruh Pengeluaran', 1, 0, 'L'); 
		$pdf->Cell(85, 7, "Rp. ".str_replace(",", ".", number_format($total_pembelian)), 1, 0, 'L');
		$pdf->Ln();
