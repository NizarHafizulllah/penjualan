...
class Penjualan extends MY_Controller 
{
								...
								if($master)
								{
									...
									foreach($_POST['kode_barang'] as $k)
									{
										if( ! empty($k))
										{
											...
											$insert_detail	= $this->m_penjualan_detail->insert_detail($id_master, $id_barang, $jumlah_beli, $harga_satuan, $diskon, $sub_total);
											if($insert_detail)
											{
	...
	public function transaksi_cetak()
	{
		...
		$pdf->Ln();
		$pdf->image(base_url('assets/img/micon.jpg'), 55, 5, 30, 30); 
		$pdf->Ln( 28 );
		$pdf->Cell(130, 5, '------------------------------------------------------------------', 0, 0, 'L');
		$pdf->Ln();
		$pdf->Cell(25, 6, 'Nota', 0, 0, 'L'); 
		$pdf->Cell(85, 6, $nomor_nota, 0, 0, 'L');
		$pdf->Ln();
		$pdf->Cell(25, 6, 'Tanggal', 0, 0, 'L'); 
		$pdf->Cell(85, 6, date('d-M-Y H:i:s', strtotime($tanggal)), 0, 0, 'L');
		...
		foreach($_GET['kode_barang'] as $kd)
		{
				...
				$pdf->Cell(40, 7, $nama_barang, 0, 0, 'L');
				$pdf->Cell(30, 7, str_replace(',', '.', number_format($_GET['harga_satuan'][$no])), 0, 0, 'L');
				
				$pdf->Cell(35, 7, $_GET['jumlah_beli'][$no], 0, 0, 'L');
				$pdf->Cell(80, 7, str_replace(',', '.', number_format($_GET['sub_total'][$no])), 0, 0, 'L');
				$pdf->Ln();
				$no++;
		...
		$pdf->Cell(105, 7, 'Total Harga', 0, 0, 'R');
		$pdf->Cell(25, 7, str_replace(',', '.', number_format($grand_total)), 0, 0, 'L');
		$pdf->Ln();


		$pdf->Cell(105, 7, 'Total Bayar', 0, 0, 'R');
		$pdf->Cell(25, 7, str_replace(',', '.', number_format(($grand_total - $disc_total))), 0, 0, 'L');
		$pdf->Ln();

		$pdf->Cell(105, 7, 'Bayar', 0, 0, 'R');
		$pdf->Cell(25, 7, str_replace(',', '.', number_format($cash)), 0, 0, 'L');
		$pdf->Ln();

		$pdf->Cell(105, 7, 'Kembali', 0, 0, 'R');
		$pdf->Cell(25, 7, str_replace(',', '.', number_format(($cash - $grand_total))), 0, 0, 'L');
		$pdf->Ln();

		$pdf->Cell(130, 5, '------------------------------------------------------------------', 0, 0, 'L');
		$pdf->Ln();
		$pdf->Cell(35, 6, 'Kasir', 0, 0, 'L'); 
		$pdf->Cell(85, 6, $kasir, 0, 0, 'L');
		$pdf->Ln();
		$pdf->Cell(35, 6, 'Pelanggan', 0, 0, 'L'); 
		$pdf->Cell(85, 6, $pelanggan, 0, 0, 'L');
		$pdf->Ln();
		$pdf->Cell(35, 6, 'Catatan', 0, 0, 'L');
		$pdf->Cell(130, 6, (($catatan == '') ? 'Tidak Ada' : $catatan), 0, 0, 'L');
		$pdf->Ln();

		$pdf->Cell(130, 5, '------------------------------------------------------------------', 0, 0, 'L');
		$pdf->Ln();
		$pdf->Ln();
		$pdf->Cell(130, 6, '"Na Kalupa Sia Remalik Kebali"', 0, 0, 'C');
				...
				foreach($barang->result() as $b)
				{
					...
					$json['datanya'] .= "
						<li>
							...
							<span id='diskonnya' style='display:none;'>".$b->diskon."</span>
	...
	public function update(){
		...
		$maindata = array('grand_total' => $post['grand_total'],
						'keterangan_lain' => $post['catatan'], 
						'bayar' => $post['cash'],
						'id_penjualan_m' => $post['id_penjualan_m']);

		// print_r($post);
		// exit();


		foreach ($post['jumlah_beli'] as $key => $value) {

			$datadelete[] = $post['id_penjualan_d'][$key];

			if (!empty($post['id_penjualan_d'][$key])) {
				

				$datadetail = array('id_penjualan_d' => $post['id_penjualan_d'][$key],
							'id_menu' => $this->m_barang->get_id($post['kode_barang'][$key])->row()->id_barang,
							'diskon' => $post['diskon'][$key],
							'harga_satuan' => $post['harga_satuan'][$key],
							'jumlah_beli' => $value, 
							'total' => $post['sub_total'][$key],
							'id_penjualan_m' => $post['id_penjualan_m']);
				...
				$this->db->where('id_penjualan_d', $post['id_penjualan_d'][$key]);
				$datasebelumnya = $this->db->get('pj_penjualan_detail')->row_array();


				// echo $datasebelumnya['jumlah_beli'];
				$jumlah_beli = $value-$datasebelumnya['jumlah_beli'];

				// echo $jumlah_beli;
				if ($jumlah_beli>0) {
					$this->m_barang->update_stok($datasebelumnya['id_menu'], $jumlah_beli);
					
				}else if ($jumlah_beli<0) {
					$jumlah_beli = abs($jumlah_beli);
					$this->m_barang->update_stok2($datasebelumnya['id_menu'], $jumlah_beli);

				}


				$this->db->where('id_penjualan_d', $post['id_penjualan_d'][$key]);
				$update = $this->db->update('pj_penjualan_detail', $datadetail);
				...
				$datadetail = array(
							...
							'diskon' => $post['diskon'][$key],
							'harga_satuan' => $post['harga_satuan'][$key],
							'jumlah_beli' => $post['jumlah_beli'][$key], 
							'total' => $post['sub_total'][$key], 
							'id_penjualan_m' => $post['id_penjualan_m']);
				$insert = $this->db->insert('pj_penjualan_detail',$datadetail);
				// echo $this->db->last_query();
				$datadelete[] = $this->db->insert_id();
				// print_r($datadetail);
				$this->m_barang->update_stok($datadetail['id_menu'], $datadetail['jumlah_beli']);
			}
		...
		$this->db->where('id_penjualan_m', $maindata['id_penjualan_m']);
		$save = $this->db->update('pj_penjualan_master', $maindata);

		if ($save) {
		
				if (!empty($datadelete)) {
					$this->db->where('id_penjualan_m', $post['id_penjualan_m']);
					$this->db->where_not_in('id_penjualan_d', $datadelete);
					$rowdelete = $this->db->get('pj_penjualan_detail');
				
				if ($rowdelete->num_rows()>0) {
					$deletedbahan = $rowdelete->result_array();
				
				foreach ($deletedbahan as $key) {
						

						$this->m_barang->update_stok2($key['id_menu'], $key['jumlah_beli']);
						// echo $this->db->last_query();
					}
				}
				...
				$this->db->where('id_penjualan_m', $post['id_penjualan_m']);
				$this->db->where_not_in('id_penjualan_d', $datadelete);
				$delete = $this->db->delete('pj_penjualan_detail');
				// exit();
	...
	public function edit(){
		...
		$dt['m_transaksi'] = $this->m_penjualan_master->get_baris($post['id'])->row_array();
		
		$dt['detail_transaksi'] = $this->m_penjualan_master->get_detail($post['id']);
		...
		// exit();
		$this->load->view('penjualan/penjualan_edit', $dt);
		// echo $this->db->last_query();
	...
	public function history_json()
	{
		...
		{ 
			...
			$nestedData[]	= "<a href='".site_url('penjualan/edit?id='.$row['id_penjualan_m'])."'><i class='fa fa-file-text-o fa-fw'></i> ".$row['nomor_nota']."</a>";
